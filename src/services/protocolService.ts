...
class ProtocolService {
  ...
  async generateProtocolFromAI(
    ...
    try {
      ...
      errorLogger.logError((error as Error).message, {
        action: 'Generate Protocol From AI',
        metadata: { projectId, researchQuestion, frameworkType }
      })
      throw error
    }
  }
  ...
  private async parseAIGuidance(guidance: string, frameworkType: 'pico' | 'spider' | 'other'): Promise<Partial<ProtocolInsert>> {
    ...
    try {
      ...
      errorLogger.logError((error as Error).message, {
        action: 'Parse AI Guidance',
        metadata: { guidance: guidance.substring(0, 200), frameworkType }
      })
      return {}
    }
  }
  ...
  private async parseAIRefinements(
    ...
    try {
      ...
      errorLogger.logError((error as Error).message, {
        action: 'Parse AI Refinements',
        metadata: { focusArea, guidance: guidance.substring(0, 200) }
      })
    }
  ...
  async duplicateProtocol(id: string, newTitle?: string): Promise<Protocol> {
    try {
      ...
      errorLogger.logError((error as Error).message, {
        action: 'Duplicate Protocol',
        metadata: { id, newTitle }
      })
      throw error
    }
  }
